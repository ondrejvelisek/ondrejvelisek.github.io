...
function DisplayIndicators() {
  ...
  return (
    <Stack
      ...
      className="w-full"
    >
      <Text scale="indicator" tone={view.angleMode === 'rad' ? 'accent' : 'muted'}>
        {ANGLE_MODE_LABEL[view.angleMode]}
      </Text>
      {view.isSecondMode && (
        <Text scale="indicator" tone="accent" aria-label={ARIA_LABELS.secondModeIndicator}>
          2nd
        </Text>
      )}
      {view.hasMemory && (
...
function DisplayExpression() {
  ...
  return (
    ...
    >
      {view.settings.keepExpression ? view.expression : ''}
    </Text>
  );
}
