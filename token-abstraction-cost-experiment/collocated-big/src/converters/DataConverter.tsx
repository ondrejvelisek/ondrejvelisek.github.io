...
const UNITS = [
  ...
  { id: 'kb', label: 'kilobyte', toBase: 1000 },
  { id: 'mb', label: 'megabyte', toBase: 1000000 },
  { id: 'gb', label: 'gigabyte', toBase: 1000000000 },
  { id: 'tb', label: 'terabyte', toBase: 1000000000000 },
  { id: 'kib', label: 'kibibyte', toBase: 1024 },
  { id: 'mib', label: 'mebibyte', toBase: 1048576 },
  { id: 'gib', label: 'gibibyte', toBase: 1073741824 },
] as const;
...
export function DataConverter() {
  ...
  const [from, setFrom] = useState<UnitId>('mb');
  const [to, setTo] = useState<UnitId>('mib');
  ...
  return (
    <section className="flex flex-col gap-2 rounded-2xl bg-white p-4 ring-1 ring-slate-300
      ...
      <h3 className="text-xs font-semibold tracking-wide text-indigo-700 uppercase dark:text-indigo-300">Data</h3>
      ...
      <div className="flex items-center gap-2">
        <output className="w-24 truncate rounded-lg bg-slate-100 px-2 py-1 text-right text-sm tabular-nums text-indigo-700 ring-1 ring-slate-300 dark:bg-slate-950/70 dark:text-indigo-200 dark:ring-slate-700/60">
          {converted}
        </output>
        <select
          value={to}
          onChange={(event) => setTo(event.target.value as UnitId)}
          aria-label="Data to unit"
          className="flex-1 rounded-lg bg-slate-100 px-2 py-1 text-xs text-slate-700 ring-1 ring-slate-300 dark:bg-slate-800/70 dark:text-slate-200 dark:ring-slate-700/60"
        >
          {UNITS.map((candidate) => (
            <option key={candidate.id} value={candidate.id}>
