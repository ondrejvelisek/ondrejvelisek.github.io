...
const UNITS = [
  ...
  { id: 'h', label: 'hour', toBase: 3600 },
  { id: 'd', label: 'day', toBase: 86400 },
  { id: 'wk', label: 'week', toBase: 604800 },
] as const;
...
export function TimeConverter() {
  ...
  const [from, setFrom] = useState<UnitId>('s');
  const [to, setTo] = useState<UnitId>('min');
  ...
  return (
    <section className="flex flex-col gap-2 rounded-2xl bg-white p-4 ring-1 ring-slate-300
      ...
      <h3 className="text-xs font-semibold tracking-wide text-rose-700 uppercase dark:text-rose-300">Time</h3>
      ...
      <div className="flex items-center gap-2">
        <output className="w-24 truncate rounded-lg bg-slate-100 px-2 py-1 text-right text-sm tabular-nums text-rose-700 ring-1 ring-slate-300 dark:bg-slate-950/70 dark:text-rose-200 dark:ring-slate-700/60">
          {converted}
        </output>
        <select
          value={to}
          onChange={(event) => setTo(event.target.value as UnitId)}
          aria-label="Time to unit"
          className="flex-1 rounded-lg bg-slate-100 px-2 py-1 text-xs text-slate-700 ring-1 ring-slate-300 dark:bg-slate-800/70 dark:text-slate-200 dark:ring-slate-700/60"
        >
          {UNITS.map((candidate) => (
            <option key={candidate.id} value={candidate.id}>
