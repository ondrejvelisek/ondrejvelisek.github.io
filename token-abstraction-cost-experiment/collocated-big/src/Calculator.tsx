      ...
      <main className="flex min-h-dvh items-center justify-center bg-slate-100 p-6 dark:bg-slate-950">
        <div className="flex w-full max-w-5xl flex-col gap-4 lg:flex-row">
          <section className="flex flex-1 flex-col gap-4 rounded-3xl bg-white p-5 ring-1 ring-slate-300 dark:bg-slate-900/80 dark:ring-slate-700/50">
            ...
            <button
              type="button"
              onClick={() => setShowConverters((shown) => !shown)}
              aria-pressed={showConverters}
              className="self-start text-xs text-slate-500 transition-colors hover:text-cyan-700 dark:hover:text-cyan-300"
            >
              {showConverters ? 'Hide converters' : 'Show converters'}
            </button>
            {showConverters && <ConverterPanel />}
          </section>
          <History />
        </div>
      </main>
